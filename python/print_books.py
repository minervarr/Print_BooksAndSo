...
Usage:
    python3 python/print_books.py init BOOK.pdf [-o project.toml]
    python3 python/print_books.py build project.toml [-o out.pdf]
    PRINT_BOOKS_BIN=/path/to/print-books python3 python/print_books.py …
"""
...
def _is_this_script(path: Path) -> bool:
    """True if *path* is this launcher (would recurse under execv)."""
    try:
        return path.resolve().samefile(_SCRIPT)
    except OSError:
        return False
...
def find_binary() -> str | None:
    ...
    Search order (first hit wins):
      ...
      2. a PATH entry named print-books that is not this script
      3. well-known build trees under the repo root (parent of python/)
    """
    env = os.environ.get("PRINT_BOOKS_BIN")
    if env:
        candidate = Path(env)
        if _is_executable(candidate) and not _is_this_script(candidate):
